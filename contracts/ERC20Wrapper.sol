...
import 'openzeppelin-solidity/contracts/drafts/ERC20Permit.sol';
...
contract ERC20Wrapper is ERC20Permit, ERC1155Receiver {
    ...
    constructor(
        ...
        uint8 _decimals
    ) public ERC20(_name, _symbol) ERC20Permit(_name) {
     ...
     * Requirements:
     *
     * - if the _msgSender() is not augurFoundry then it needs to have given setApprovalForAll
     *  to this contract (if the _msgSender() is augur foundry then we trust it and know that
     ...
     * Requirements:
     *
     * - if the _msgSender() is not augurFoundry or _account then the caller must have allowance for ``_account``'s tokens of at least
     * `amount`.
     * - if the market has finalized then claim() function should be called.
     * @param _account  account who's ERC20 will be burned
     ...
     * Requirements:
     *  - if _msgSender() is not {_account} then {_account} should have given allowance to _msgSender()
